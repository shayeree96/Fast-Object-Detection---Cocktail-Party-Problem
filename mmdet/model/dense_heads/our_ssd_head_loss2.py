...
class OURSSDHead_Loss2(AnchorHead):
    ...
    def loss_single(self, cls_score, bbox_pred, anchor, labels, label_weights,
        ...
        ######################this part is the only difference from our_ssd_head #########################
        softmax_cls_score = self.softmax(cls_score)
        position_prob_1 = softmax_cls_score[torch.arange(softmax_cls_score.size(0)), labels[0]]
        loss_cls_all1 = F.cross_entropy(
            cls_score, labels[0], reduction='none') * label_weights[0]

        position_prob_2 = softmax_cls_score[torch.arange(softmax_cls_score.size(0)), labels[1]]
        loss_cls_all2 = F.cross_entropy(
            cls_score, labels[1], reduction='none') * label_weights[1]

        loss_cls_all = torch.pow((0.5-position_prob_1), 2) * loss_cls_all1 + torch.pow((0.5-position_prob_2), 2) * loss_cls_all2
        #################################################################################################
